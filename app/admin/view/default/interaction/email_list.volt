...
{% block content %}
    ...
    <div class="row">
        ...
        <div class="col-sm-12 m-b-20">
            <a href="{{ url({'for': 'interaction_add_email'}) }}" class="btn btn-primary float-right">Email</a>
                    ...
                    {% if result is defined and result|length %}
                        {% for item in result %}
                            <tr>
                                ...
                                <td>
                                    <a role="menuitem" tabindex="-1" href="{{ url({'for': 'interaction_edit_email', 'query': '?' ~ http_build_query({'id': item.id})}) }}">
                                        {{ item.name }}
                                    </a>
                                </td>
                                <td>
