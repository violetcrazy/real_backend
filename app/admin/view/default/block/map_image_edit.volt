    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="page-header">
                <h3>Chỉnh sửa hình ảnh hiển thị</h3>
    ...
    <form id="form-article" action="" method="POST" enctype="multipart/form-data" class="form-horizontal">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-11 block-upload-img-view">
                <div class="thumbnail-img">
                    <img src="{{ config.asset.frontend_url ~ 'upload/block/' ~ mapImage.image }}" alt="" />
                    <input type="file" class="hidden" accept="image/*" value="" />
                    <input type="hidden" class="input_image" name="image_view" value="{{ mapImage.image }}" />
                ...
                <select id="floor_count" class="form-control" name="floor">
                    {% if (floor_name_list|length) %}
                        {% for index, value in floor_name_list %}
                            <option {{ mapImage.floor is defined and mapImage.floor == index ? 'selected' : '' }} value="{{ index }}">
                                {{ value }}
                            </option>
                        {% endfor %}
                    {% else %}
                        {% for index, value in 1..result['floor_count'] %}
                            <option {{ mapImage.floor is defined and mapImage.floor == index ? 'selected' : '' }} value="{{ value }}">
                                {{ value }}
                            </option>
                        {% endfor %}
                    {% endif %}
        ...
        <div class="form-group">
            ...
            <div class="col-sm-11 text-right">
                ...
                <a href="{{ url({'for': 'block_map_image_add', 'query': '?' ~ http_build_query({'block_id': result['id']})}) }}" class="btn btn-primary">
                    Thêm mới
                </a>
