...
{% block content %}
    ...
    <div class="row">
        ...
        <div class="col-sm-12 m-b-20">
            <a href="{{ url({'for': 'category_add'}) }}" class="btn btn-primary float-right">Thêm danh mục</a>
                    ...
                    {% if categories is defined and categories|length %}
                        {% for item in categories %}
                            <tr>
                                ...
                                <td>
                                    <a role="menuitem" tabindex="-1" href="{{ url({'for': 'category_edit', 'query': '?' ~ http_build_query({'id': item.id})}) }}">
                                        {{ item.name }}
                                    </a>
                                </td>
                                <td>{{ item.name_eng }}</td>
