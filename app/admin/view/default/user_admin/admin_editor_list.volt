    ...
    <div class="row">
        <div class="col-sm-12">
            ...
            <div class="page-header">
                ...
                %}
                    <a href="{{ url({'for': 'user_add_admin', 'query': '?' ~ http_build_query({'q': q, 'filter': 'admin_editor_list'})}) }}" class="btn btn-primary pull-right">
                        <i class="fa fa-plus"></i>
                        Thêm Admin Editor
                    </a>
                {% endif %}

                <h3>Danh sách Admin Editor</h3>
            </div>
        </div>
                ...
                <tbody>
                    {% for item in result %}
                        <tr>
                            <td>{{ item.id }}</td>
                            <td>
                                <a href="{{ url({'for': 'user_edit_admin', 'query': '?' ~ http_build_query({'id': item.id, 'q': q, 'filter': 'admin_editor_list'})}) }}">
                                    {{ item.username }}
                                </a>
                            </td>
                            <td>{{ item.name }}</td>
                            ...
                            <td class="text-center">
                                {% if user_session['id'] != item['id'] %}
                                    <a href="{{ url({'for': 'user_delete_admin', 'query': '?' ~ http_build_query({'id': item.id, 'q': q, 'filter': 'admin_editor_list'})}) }}" onclick="return confirm('Đồng ý xoá?');" class="btn btn-xs btn-bricky">
