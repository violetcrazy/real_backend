                ...
                <tbody>
                    {% if result is defined and result|length %}
                        {% for item in result %}
                            <tr>
                                <td>
                                    <a href="{{ url({'for': 'user_edit_agent', 'query': '?' ~ http_build_query({'id': item['id'], 'q': q, 'filter': 'agent_list'})}) }}">
                                        {{ item['username'] }}
                                    </a>
                                </td>
                                <td>{{ item['name'] }}</td>
                                ...
                                <td class="text-center">
                                    {% if user_session['id'] != item['id'] %}
                                        <a href="{{ url({'for': 'user_delete', 'query': '?' ~ http_build_query({'id': item['id'], 'q': q, 'filter': 'agent_list'})}) }}" onclick="return confirm('Đồng ý xoá?');" class="btn btn-xs btn-bricky">
